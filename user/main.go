...
var (
	...
	mysqlUser               = common.GetEnv("MYSQL_USER", "user")
	mysqlPassword           = common.GetEnv("MYSQL_PASSWORD", "userpassword")
	mysqlHost               = common.GetEnv("MYSQL_HOST", "mysql")
	mysqlPort               = common.GetEnv("MYSQL_PORT", "3306")
	mysqlDatabase           = common.GetEnv("MYSQL_DATABASE", "testdb")
)
...
func main() {
	...
	svc.GetTierMembers(context.Background())
	svc.UpdateName(context.Background())
	svc.BanUser(context.Background())
	svc.GetEarnings(context.Background())
	svc.DeleteAccount(context.Background())
	svc.UpdateDisplayName(context.Background())
	svc.VerifyCreator(context.Background())
	svc.GetSubEarnings(context.Background())
	svc.RetrieveSub(context.Background())

	err = store.UpdateTopCreators()
	if err != nil {
		log.Printf("Error updating top creators during startup: %v", err)
	}
	...
	go func() {
		...
		for {
			...
			err := store.UpdateTopCreators()
			if err != nil {
				log.Printf("Error updating top creators: %v", err)
			}
		}
	}()

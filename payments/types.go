...
type PaymentsService interface {
	...
	RegisterChatMessage(ctx context.Context, chatMessage common.GemsOnChatPayload) (int, error)
	AcceptReq(ctx context.Context, transaction common.AcceptMessageReqPayload) error
	RejectReq(ctx context.Context, transaction common.AcceptMessageReqPayload) error
	//Grpc
...
type PaymentsStore interface {
	...
	CreateMessageGem(ctx context.Context, messageGem *MessageGem) (int, error)
	...
	ActivateSubscription(userID primitive.ObjectID, productID string, TransactionIDRC string) error
	RenewActivateSubscription(userID primitive.ObjectID, productID string, TransactionIDRC string) error
	CancelActivateSubscription(userID primitive.ObjectID, productID string, TransactionIDRC string) error
	...
	UserSubscriberCount(userID primitive.ObjectID, increase_decrease int) error
	SetMessageGemValid(transactionID int64) error
	DeleteMessageGemAndGetSenderIDAndGemAmount(transactionID int64) (string, int, error)
}
...
type RCJSONCancel struct {
	ApiVersion string      `json:"api_version"`
	Event      EventCancel `json:"event"`
}

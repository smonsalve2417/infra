...
func (s *store) ChatGemExists(senderID string, receiverID string) (bool, error) {
	query := `
        SELECT COUNT(*)
        FROM chat_gems
        WHERE sender_id = ? AND receiver_id = ? AND valid = true
    `
...
func (s *store) CreateMessageGem(ctx context.Context, messageGem *MessageGem) (int, error) {
	query := `
	...
	if err != nil {
		return 0, fmt.Errorf("error creating message gem in sql: %v", err)
	}
...
func (s *store) ActivateSubscription(userID primitive.ObjectID, productID string, TransactionIDRC string) error {
	collection := s.database.Collection("user-subscriptions")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	update := bson.M{
		"$set": bson.M{
			...
			"valid":                   true,
			"original_transaction_id": TransactionIDRC,
		},
	}
	...
	if result.MatchedCount == 0 {
		return fmt.Errorf("no subscription updated for user %s with productID %s", userID.Hex(), productID)
	}

	return nil
}

func (s *store) RenewActivateSubscription(userID primitive.ObjectID, productID string, TransactionIDRC string) error {
	collection := s.database.Collection("user-subscriptions")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	if time.Since(subscription.UpdatedAt) <= 10*24*time.Hour && subscription.ProductID == productID {
		return fmt.Errorf("subscription was updated within the last 10 days for product %s, please try again later", productID)
	}
...
func (s *store) CancelActivateSubscription(userID primitive.ObjectID, productID string, TransactionIDRC string) error {
	collection := s.database.Collection("user-subscriptions")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	// Filter by userID, productID, and original_transaction_id
	filter := bson.M{
		"user_id":                 userID,
		"productID":               productID,
		"original_transaction_id": TransactionIDRC,
	...
	// Find the subscription document to ensure it exists
	var subscription common.Subscription
	err := collection.FindOne(ctx, filter).Decode(&subscription)
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return fmt.Errorf("no subscription found for user %s with productID %s and transaction ID %s", userID.Hex(), productID, TransactionIDRC)
		}
	...
	log.Printf("Subscription for user %s with productID %s and transaction ID %s successfully deleted", userID.Hex(), productID, TransactionIDRC)
	return nil
}

func (s *store) CreateSubPurchase(purchase *SubPurchase) error {
	// Step 1: Check if a transaction with the same rc_transaction_id already exists within the last 10 days
	existsQuery := `
        SELECT created_at 
        FROM sub_purchases 
        WHERE rc_transaction_id = ? 
        AND created_at >= DATE_SUB(NOW(), INTERVAL 10 DAY)
    `

	// Set a context timeout for the existence check
	ctx, cancel := context.WithTimeout(context.Background(), QueryTimeoutDuration)
	defer cancel()

	var createdAt time.Time
	err := s.db.QueryRowContext(ctx, existsQuery, purchase.RcTransactionID).Scan(&createdAt)
	if err != nil && err != sql.ErrNoRows {
		return fmt.Errorf("error checking existing transaction: %v", err)
	}

	// If a row already exists and the transaction was created within the last 10 days, return an error
	if !createdAt.IsZero() {
		log.Printf("Transaction with ID %s already exists and was created within the last 10 days, skipping insert", purchase.RcTransactionID)
		return fmt.Errorf("transaction with ID %s was created recently (within the last 10 days)", purchase.RcTransactionID)
	}

	// Step 2: If the transaction doesn't exist, insert the new subscription purchase
	insertQuery := `
	...
	log.Printf("Transaction with ID %s successfully inserted", purchase.RcTransactionID)
	return nil
}
...
func (s *store) UserSubscriberCount(userID primitive.ObjectID, increase_decrease int) error {
	collection := s.database.Collection("users")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	filter := bson.M{"_id": userID}
	update := bson.M{"$inc": bson.M{"subscribers": increase_decrease}}
...
func (s *store) SetMessageGemValid(transactionID int64) error {
	query := `
        UPDATE message_gems 
        SET valid = TRUE 
	...
	_, err := s.db.ExecContext(ctx, query, transactionID)
	if err != nil {
		return fmt.Errorf("error setting message gem as valid in sql: %v", err)
	}

	return nil
}

func (s *store) DeleteMessageGemAndGetSenderIDAndGemAmount(transactionID int64) (string, int, error) {
	var senderID string
	var gemAmount int

	// First, retrieve the sender_id and gem_amount for the given transaction_id
	querySelect := `
        SELECT sender_id, gem_amount 
        FROM message_gems 
        WHERE transaction_id = ?
    `
	err := s.db.QueryRowContext(ctx, querySelect, transactionID).Scan(&senderID, &gemAmount)
	if err != nil {
		return "", 0, fmt.Errorf("error fetching sender_id and gem_amount: %v", err)
	}

	// Then, delete the row with the specified transaction_id
	queryDelete := `
        DELETE FROM message_gems 
        WHERE transaction_id = ?
    `
	_, err = s.db.ExecContext(ctx, queryDelete, transactionID)
	if err != nil {
		return "", 0, fmt.Errorf("error deleting message gem in sql: %v", err)
	}

	return senderID, gemAmount, nil
}

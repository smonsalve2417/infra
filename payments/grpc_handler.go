...
func (h *grpcHandler) CreateSubscribeToCreator(ctx context.Context, p *pb.CreateSubscribeToCreatorRequest) (*pb.CreateSubscribeToCreatorResponse, error) {
	log.Printf("New CreateSubscribeToCreator received! from: %v", p.UserID)
	...
	log.Printf("New sub created from %v to %v with productid %v", p.UserID, creatorObjID, p.ProductID)

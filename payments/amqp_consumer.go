...
func (c *consumer) ListenAcceptMessage(ch *amqp.Channel) {
	...
	go func() {
		for msg := range msgs {
			...
			// Intentar registrar el comentario en el servicio
			transID, err := c.service.RegisterChatMessage(context.Background(), chatMessage)
...
func (c *consumer) ListenAcceptReq(ch *amqp.Channel) {
	// Declarar la cola para recibir comentarios
	q, err := ch.QueueDeclare(broker.SendAcceptMessageReqCreatedEvent, true, false, false, false, nil)
	if err != nil {
		log.Fatal(err)
	}
	...
	go func() {
		for msg := range msgs {
			...
			// Deserializar el comentario recibido
			var Payload common.AcceptMessageReqPayload
			...
			// Intentar registrar el comentario en el servicio
			err = c.service.AcceptReq(context.Background(), Payload)
...
func (c *consumer) ListenRejectReq(ch *amqp.Channel) {
	// Declarar la cola para recibir comentarios
	q, err := ch.QueueDeclare(broker.SendRejectMessageReqCreatedEvent, true, false, false, false, nil)
	if err != nil {
		log.Fatal(err)
	}
	...
	go func() {
		for msg := range msgs {
			...
			// Intentar registrar el comentario en el servicio
			err = c.service.RejectReq(context.Background(), Payload)

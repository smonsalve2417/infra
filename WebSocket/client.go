...
func (c *Client) readMessage(hub *Hub) {
	...
	for {
		...
		if payload.Type != "message" {
			msg.Pending = true
			mongoMessage.Pending = true
		}

		err = c.handleMessageSendingLogic(hub, isRequestTrue, chatUserID, payload, mongoMessage, msg, rabbitPayload)
		if err != nil {
			log.Printf("error in handleMessageSendingLogic: %v", err)
			continue
		}
		...
		case d := <-msgs:
			if d.CorrelationId == corrId {
				...
				if response.Success {
					...
					msg.SQLid = response.TransactionID
					mongoMessage.SQLid = response.TransactionID
...
func (c *Client) handleMessageSendingLogic(hub *Hub, isRequestTrue bool, chatUserID primitive.ObjectID, payload MessagePayload, mongoMessage MongoMessage, msg *Message, rabbitPayload common.GemsOnChatPayload) error {
	if isRequestTrue {
		...
		if c.UserID == chatUserID {
			...
			if err != nil {
				...
				c.sendErrorMessage("Error while processing your request.")
				return err
			}
			...
			if messageCount >= 1 {
				...
				c.sendErrorMessage("You are not allowed to send more than one free message in this chat.")
				return nil
			}
			...
			if err != nil {
				...
				c.sendErrorMessage(fmt.Sprintf("Error during message creation: %v", err))
				return err
			}
			return nil
		}
	...
	if payload.Gems == 0 {
		if c.UserID == chatUserID {
			...
			c.sendErrorMessage("Cant send a message with no gems.")
			return nil
		}
	}
	...
	if c.UserID == chatUserID {
		...
		if err != nil {
			...
			c.sendErrorMessage(fmt.Sprintf("Error in message processing: %v", err))
			return err
		}
	} else {
		log.Print("no toca pagar")

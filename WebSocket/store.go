...
func ConvertMongoMessageToMessage(mongoMsg MongoMessage) Message {
	return Message{
		...
		Type:      mongoMsg.Type,
		Pending:   mongoMsg.Pending,
		SQLid:     mongoMsg.SQLid,
	}
}
...
func (s *store) ConvertToGrpcMessage(message *Message) *pb.Message {
	return &pb.Message{
		...
		TransactionId: int32(message.SQLid),
	}
}
...
func (s *store) UpdateMessageRequestStatus(transactionID int) error {
	collection := s.database.Collection("messages")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	// Define the filter to locate the document to update
	filter := bson.M{"sqlid": transactionID}
...
func (s *store) DeleteMessageByTransactionID(transactionID int) error {
	collection := s.database.Collection("messages")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	if err != nil {
		return fmt.Errorf("error deleting document with transactionID %d: %v", transactionID, err)
	}

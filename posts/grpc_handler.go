...
func (h *grpcHandler) DeletePost(ctx context.Context, p *pb.DeletePostRequest) (*pb.DeletePostResponse, error) {
	...
	if err != nil {
		o := &pb.DeletePostResponse{}
		return o, err
	}

	if post.User_id != userObjID && !h.store.IsAdmin(*User) {
		o := &pb.DeletePostResponse{Status: "no authorization to delete"}
		err = fmt.Errorf("only the owner or an admin can delete the post")
		return o, err
	}
...
func (h *grpcHandler) CreateComment(ctx context.Context, p *pb.CreateCommentRequest) (*pb.CreateCommentResponse, error) {
	...
	if p.Comment.Gems == 0 {
		NumberOfComments, err := h.store.CountCommentsByUserAndPost(userObjID, postObjID)
		if err != nil {
			return nil, err
		}
		if NumberOfComments < 3 {
			...
			err = fmt.Errorf("exeded amount of free coments")
			return nil, err
		}
	}
...
func (h *grpcHandler) DeleteComment(ctx context.Context, p *pb.DeleteCommentRequest) (*pb.DeleteCommentResponse, error) {
	...
	commentOwner, err := h.store.GetUserIDFromComment(commentObjID)
	if err != nil {
		o := &pb.DeleteCommentResponse{Status: "error deleting comment process: " + err.Error()}
		return o, err
	}

	postOwner, err := h.store.GetPostOwnerFromComment(commentObjID)
	if err != nil {
		o := &pb.DeleteCommentResponse{Status: "error deleting comment process: " + err.Error()}
		return o, err
	}
	...
	User, err := h.store.GetUserByID(p.UserID)
	if err != nil {
		o := &pb.DeleteCommentResponse{}
		return o, err
	}

	if commentOwner != userObjID && postOwner != userObjID && !h.store.IsAdmin(*User) {
		o := &pb.DeleteCommentResponse{Status: "no authorization to delete"}
		err = fmt.Errorf("only the comment owner, post owner, or an admin can delete the comment")
		return o, err
	}
...
func (h *grpcHandler) GetLatestPosts(ctx context.Context, p *pb.GetLatestPostsRequest) (*pb.GetLatestPostsResponse, error) {
	...
	list, err := h.store.GetLatestsPosts(userObjID, int(p.Page), 10)
	if err != nil {
		o := &pb.GetLatestPostsResponse{}
		return o, err
	}
	...
	for _, postwithuser := range list {
		...
		o := &pb.PostWithUser{
			Post:   grpcPost,
			User:   grpcUser,
			Subbed: postwithuser.Subscribed,
		}
...
func (h *grpcHandler) GetLatestUserPosts(ctx context.Context, p *pb.GetLatestUserPostsRequest) (*pb.GetLatestUserPostsResponse, error) {
	...
	list, err := h.store.GetLatestsUserPosts(userObjID, objUserID, int(p.Page), 10)
	if err != nil {
		o := &pb.GetLatestUserPostsResponse{}
		return o, err
	}
...
func (h *grpcHandler) GetPost(ctx context.Context, p *pb.GetPostRequest) (*pb.GetLatestPostsResponse, error) {
	log.Printf("New GetPosts received!")
	...
	list, err := h.store.GetPost(postObjID, userObjID)
	if err != nil {
		o := &pb.GetLatestPostsResponse{}
		return o, err
	}
...
func (h *grpcHandler) GetRecommended(ctx context.Context, p *pb.GetRecommendedRequest) (*pb.GetRecommendedResponse, error) {
	log.Printf("New GetRecommended received!")

	userObjID, err := primitive.ObjectIDFromHex(p.UserID)
	if err != nil {
		o := &pb.GetRecommendedResponse{}
		return o, err
	}

	list, err := h.store.GetRecommended(userObjID)
	if err != nil {
		o := &pb.GetRecommendedResponse{}
		return o, err
	}
...
func (h *grpcHandler) SharePost(ctx context.Context, p *pb.SharePostRequest) (*pb.SharePostResponse, error) {
	log.Printf("New SharePost received! Post from: %v", p.PostID)

	objUserID, err := primitive.ObjectIDFromHex(p.UserID)
	if err != nil {
		o := &pb.SharePostResponse{}
		return o, err
	}
	...
	err = h.store.SharePost(objUserID, objPostID)
	if err != nil {
		o := &pb.SharePostResponse{}
		return o, err
	}

	o := &pb.SharePostResponse{Status: "successfull post share"}
	return o, nil
}

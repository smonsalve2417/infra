...
func (s *service) RegisterChatMessage(ctx context.Context, chatMessage common.GemsOnChatPayload) error {
	...
	messageGem := &MessageGem{
		...
		Valid:      true, // Default is true, but can be set manually
		GemAmount:  chatMessage.Gems,
	}

	err = s.store.CreateMessageGem(context.Background(), messageGem)
	if err != nil {

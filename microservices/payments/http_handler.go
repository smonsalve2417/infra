...
func (h *handler) registerRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /initialPurchase", h.InitialPurchase)
...
func (h *handler) InitialPurchase(w http.ResponseWriter, r *http.Request) {
	...
	err = h.store.ActivateSubscription(user.ID, rcData.Event.ProductID)
	if err != nil {
...
func (h *handler) Renewal(w http.ResponseWriter, r *http.Request) {
	...
	err = h.store.RenewActivateSubscription(user.ID, rcData.Event.ProductID)
	if err != nil {
		log.Println("Error activating subscription:", err)
		common.WriteJSON(w, http.StatusBadRequest, err)
	}
...
func (h *handler) Cancellation(w http.ResponseWriter, r *http.Request) {
	...
	err = h.store.CancelActivateSubscription(user.ID, rcData.Event.ProductID)
	if err != nil {

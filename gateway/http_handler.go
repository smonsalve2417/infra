...
func (h *handler) registerRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /posts/sharePost", auth.WithJWTAuth(h.SharePost, h.mongodatabase))
	//		comments
	...
	mux.HandleFunc("GET /posts/getRecommended/{page}", auth.WithJWTAuth(h.GetRecommended, h.mongodatabase))
	//User
	//		auth
	...
	mux.HandleFunc("POST /auth/verifyCreator", auth.WithJWTAuth(h.VerifyCreator, h.mongodatabase))
	...
	mux.HandleFunc("POST /user/updateName", auth.WithJWTAuth(h.HandleUpdateName, h.mongodatabase))
	mux.HandleFunc("POST /user/updateDisplayName", auth.WithJWTAuth(h.UpdateDisplayName, h.mongodatabase))
	...
	mux.HandleFunc("GET /user/getSubscriptionTiers/{creatorID}", auth.WithJWTAuth(h.GetSubscriptionTiers, h.mongodatabase))
	mux.HandleFunc("GET /user/getTierMembers/{creatorID}/{tier}/{page}", auth.WithJWTAuth(h.GetTierMembers, h.mongodatabase))
	mux.HandleFunc("POST /user/banUser", auth.WithJWTAuth(h.BanUser, h.mongodatabase))
	mux.HandleFunc("GET /user/getEarnings", auth.WithJWTAuth(h.GetEarnings, h.mongodatabase))
	mux.HandleFunc("DELETE /user/deleteAccount", auth.WithJWTAuth(h.DeleteAccount, h.mongodatabase))
	mux.HandleFunc("GET /user/getSubEarnings", auth.WithJWTAuth(h.GetSubEarnings, h.mongodatabase))
	mux.HandleFunc("GET /user/retrieveSubs/{page}", auth.WithJWTAuth(h.RetrieveSubs, h.mongodatabase))
	...
	mux.HandleFunc("POST /ws/acceptMessageReq/{transID}", auth.WithJWTAuth(h.AcceptMessageReq, h.mongodatabase))
	mux.HandleFunc("DELETE /ws/acceptMessageReq/{transID}", auth.WithJWTAuth(h.RejectMessageReq, h.mongodatabase))

	//Payments
	mux.HandleFunc("GET /payment/getAvailableSubscriptionGroup", auth.WithJWTAuth(h.GetAvailableSubscriptionGroup, h.mongodatabase))
	mux.HandleFunc("POST /payment/createSubscribeToCreator", auth.WithJWTAuth(h.CreateSubscribeToCreator, h.mongodatabase))
}
...
func (h *handler) SharePost(w http.ResponseWriter, r *http.Request) {
	...
	var payload common.SharePostPostPayload
	...
	o, err := h.postClient.SharePost(r.Context(), &pb.SharePostRequest{
		PostID: payload.Post_id.Hex(),
		UserID: userID.Hex(),
	})
...
func (h *handler) GetRecommended(w http.ResponseWriter, r *http.Request) {
	...
	o, err := h.postClient.GetRecommended(r.Context(), &pb.GetRecommendedRequest{
		Page:   pageNum32,
		UserID: userID.Hex(),
	})
...
func (h *handler) VerifyCreator(w http.ResponseWriter, r *http.Request) {
	userID, err := auth.GetUserIDFromContext(r.Context())
	if err != nil {
		common.WriteError(w, http.StatusUnauthorized, "unauthorized: "+err.Error())
		return
	...
	var payload common.VerifyCreatorPayload
	...
	o, err := h.userClient.VerifyCreator(r.Context(), &pb.VerifyCreatorRequest{
		UserID:         userID.Hex(),
		InstagramToken: payload.InstagramAccessToken,
	})
...
func (h *handler) HandleUpdateName(w http.ResponseWriter, r *http.Request) {
	...
	var payload common.ChangeNamePayload
	...
	o, err := h.userClient.UpdateName(r.Context(), &pb.UpdateNameRequest{
		FirstName: payload.FirstName,
		LastName:  payload.LastName,
		UserID:    userID.Hex(),
	})
...
func (h *handler) UpdateDisplayName(w http.ResponseWriter, r *http.Request) {
	...
	var payload common.ChangeDisplayNamePayload
	...
	o, err := h.userClient.UpdateDisplayName(r.Context(), &pb.UpdateDisplayNameRequest{
		UserID:      userID.Hex(),
		DisplayName: payload.DisplayName,
	})
...
func (h *handler) GetTierMembers(w http.ResponseWriter, r *http.Request) {
	creatorID := r.PathValue("creatorID")
	...
	o, err := h.userClient.GetTierMembers(r.Context(), &pb.GetTierMembersRequest{
		CreatorId: creatorID,
		Tier:      int32(tiernum),
		Page:      int32(pagenum),
	})
...
func (h *handler) BanUser(w http.ResponseWriter, r *http.Request) {
	...
	var payload common.BanUserPayload
	if err := common.ParseJSON(r, &payload); err != nil {
		log.Printf("Error parsing JSON: %v", err)
		common.WriteError(w, http.StatusBadRequest, err.Error())
		return
	...
	o, err := h.userClient.BanUser(r.Context(), &pb.BanUserRequest{
		UserId:   userID.Hex(),
		TargetId: payload.TargetID.Hex(),
	})
...
func (h *handler) GetEarnings(w http.ResponseWriter, r *http.Request) {
	...
	o, err := h.userClient.GetEarnings(r.Context(), &pb.GetEarningsRequest{
		UserId: userID.Hex(),
	})
...
func (h *handler) DeleteAccount(w http.ResponseWriter, r *http.Request) {
	...
	o, err := h.userClient.DeleteAccount(r.Context(), &pb.DeleteAccountRequest{
		UserId: userID.Hex(),
	})
...
func (h *handler) GetSubEarnings(w http.ResponseWriter, r *http.Request) {
	userID, err := auth.GetUserIDFromContext(r.Context())
	if err != nil {
		common.WriteError(w, http.StatusUnauthorized, "unauthorized: "+err.Error())
		return
	...
	o, err := h.userClient.GetSubEarnings(r.Context(), &pb.GetSubEarningsRequest{
		UserId: userID.Hex(),
	})
...
func (h *handler) RetrieveSubs(w http.ResponseWriter, r *http.Request) {
	userID, err := auth.GetUserIDFromContext(r.Context())
	if err != nil {
		common.WriteError(w, http.StatusUnauthorized, "unauthorized: "+err.Error())
		return
	...
	o, err := h.userClient.RetrieveSub(r.Context(), &pb.RetrieveSubRequest{
		UserID: userID.Hex(),
		Page:   int32(pagenum),
	})
...
func (h *handler) AcceptMessageReq(w http.ResponseWriter, r *http.Request) {

	transID := r.PathValue("transID")
	transIDnum, err := strconv.Atoi(transID)
	if err != nil {
		fmt.Println("Error converting string to int:", err)
		common.WriteError(w, http.StatusBadRequest, "invalid page number")
		return
	...
	o, err := h.webClient.AcceptChatInsideReq(r.Context(), &pb.AcceptChatInsideReqRequest{
		TransID: int32(transIDnum),
	})
...
func (h *handler) RejectMessageReq(w http.ResponseWriter, r *http.Request) {
	...
	o, err := h.webClient.RejectChatInsideReq(r.Context(), &pb.AcceptChatInsideReqRequest{
		TransID: int32(transIDnum),
	})

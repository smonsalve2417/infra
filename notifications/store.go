...
func (s *store) CreatePostNotifications(postID primitive.ObjectID, userID primitive.ObjectID, notificationType string) error {
	...
	for _, followerID := range followers {
		...
		followerUser, err := s.GetUserNotPublicByID(followerID)
		if err != nil {
			return fmt.Errorf("error getting user: %v", err)
		}
		...
		if post.User_id != followerID && post.Tier != 0 {
			userTier, err := s.GetSubscriptionTier(followerID, post.User_id)
			if err != nil {
				return err
			}
			if userTier < post.Tier && !s.IsAdmin(*followerUser) {
		...
		notification := bson.M{
			"user": bson.M{
				...
				"displayName": userCreatingPost.DisplayName,
...
func (s *store) GetUserNotPublicByID(userID primitive.ObjectID) (*common.User, error) {
	collection := s.database.Collection("users")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (s *store) GetLatestsNotifications(page int, pageSize int, userID primitive.ObjectID) ([]NotificationMongo, error) {
	...
	for i, notif := range Notifications {
		...
		var subbed = true
		notif.Post.Content = notif.Post.BluredContent

		if notif.Post.Tier != 0 {
			userTIer, err := s.GetSubscriptionTier(userID, notif.Post.User_id)
			if err != nil {
				return nil, err
			}

			if userTIer < notif.Post.Tier {
...
func ConvertToGrpcNotification(notificationMongo NotificationMongo) *pb.Notification {
	return &pb.Notification{
		...
		User: &pb.PublicUser{
			...
			DisplayName: notificationMongo.User.DisplayName,
		...
		Post: &pb.Post{
			...
			UserId:      notificationMongo.Post.User_id.Hex(), // Convert ObjectID to string
		...
		Subscribed:        notificationMongo.Subbed,
	}
}
